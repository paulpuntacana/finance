...
import {
  ...
  BarChart2, Euro, Settings, Edit3, X,
...
function buildMonthlyGrowth(profiles) {
  ...
  for (let i = 11; i >= 0; i--) {
    ...
    months.push({ month: d.toLocaleString('nl-NL', { month: 'short' }), date: d, new: 0, total: 0 })
  }
  profiles.forEach(p => {
...
function buildRoleDistribution(profiles) {
  ...
  return Object.entries(counts).map(([role, value]) => ({
    name: ROLE_LABELS[role]?.label || role, value, color: ROLE_LABELS[role]?.color || '#7a9cc8',
  }))
}
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border-2)', borderRadius: '16px', padding: '28px', width: '100%', maxWidth: '460px', boxShadow: '0 24px 80px rgba(0,0,0,0.4)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          ...
          <div style={{ display: 'flex', gap: '10px', marginTop: '4px' }}>
            <button onClick={save} disabled={loading} style={{ flex: 1, background: 'var(--accent)', border: 'none', borderRadius: '8px', padding: '10px', color: '#fff', fontSize: '13px', fontWeight: '600', cursor: 'pointer' }}>
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border-2)', borderRadius: '16px', padding: '28px', width: '100%', maxWidth: '460px', boxShadow: '0 24px 80px rgba(0,0,0,0.4)' }}>
        ...
        <p style={{ fontSize: '13px', color: 'var(--text-3)', margin: '0 0 18px' }}>Stuur aan <strong style={{ color: 'var(--text)' }}>{profile.full_name || profile.email}</strong></p>
        ...
        <div style={{ background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: '8px', padding: '10px 12px', fontSize: '11px', color: 'var(--text-3)', fontFamily: 'monospace', wordBreak: 'break-all', marginBottom: '16px' }}>{link}</div>
...
// ── MODAL: Nieuwe gebruiker aanmaken ──────────────────────────────────────────
function CreateUserModal({ onClose, onCreated, orgs }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border-2)', borderRadius: '16px', padding: '28px', width: '100%', maxWidth: '480px', boxShadow: '0 24px 80px rgba(0,0,0,0.4)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '22px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <div>
              ...
              <p style={{ margin: 0, fontSize: '12px', color: 'var(--text-3)' }}>Account aanmaken op het platform</p>
            </div>
          </div>
          <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-3)' }}><X size={18} /></button>
        </div>
        ...
        <form onSubmit={handleCreate} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          ...
          <div>
            <label style={{ display: 'block', color: 'var(--text-2)', fontSize: '11px', fontWeight: '600', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '6px' }}>Wachtwoord</label>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
            ...
            <div>
              <label style={{ display: 'block', color: 'var(--text-2)', fontSize: '11px', fontWeight: '600', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '6px' }}>Koppel aan organisatie</label>
              <select style={inp} value={form.organization_id} onChange={e => setForm(f => ({ ...f, organization_id: e.target.value }))}>
                <option value="">— Geen —</option>
                {orgs.map(o => <option key={o.id} value={o.id}>{o.name}</option>)}
              </select>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
        ...
        <Card style={{ padding: '20px' }}>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={monthlyData}>
              <defs>
                <linearGradient id="gTotal" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#3b82f6" stopOpacity={0.2} /><stop offset="95%" stopColor="#3b82f6" stopOpacity={0} /></linearGradient>
                <linearGradient id="gNew"   x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#10b981" stopOpacity={0.2} /><stop offset="95%" stopColor="#10b981" stopOpacity={0} /></linearGradient>
              </defs>
      ...
      <Card>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {[...profiles].sort((a, b) => new Date(b.created_at) - new Date(a.created_at)).slice(0, 6).map(p => (
              <tr key={p.id}>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      <Card>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr>{['Gebruiker', 'Rol', 'Organisatie', 'Status', 'Aangemeld', 'Acties'].map(h => <th key={h} style={th}>{h}</th>)}</tr>
            </thead>
            <tbody>
              {filtered.length === 0 ? (
                ...
                return (
                  <tr key={p.id} style={{ opacity: p.is_active ? 1 : 0.5 }}>
                    ...
                    <td style={td}><RoleBadge role={p.role} /></td>
                    <td style={{ ...td, fontSize: '12px', color: 'var(--text-2)' }}>
                      {org?.name || <span style={{ color: 'var(--text-3)', fontStyle: 'italic' }}>Geen</span>}
                    </td>
                    <td style={td}>
                      {p.is_active
                    ...
                    <td style={td}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                        <button onClick={() => setEditModal(p)} style={{ background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: '6px', padding: '5px 8px', color: 'var(--text-2)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '4px', fontSize: '11px' }}>
                          <Edit3 size={12} /> Bewerken
                        </button>
                        <button onClick={() => setCodeModal(p)} style={{ background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: '6px', padding: '5px 8px', color: 'var(--text-2)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '4px', fontSize: '11px' }}>
...
function OrgsTab({ orgs, setOrgs, profiles, onReload }) {
  ...
  // Link bestaande gebruiker aan org
  ...
  const linkUser = async (e) => {
    e.preventDefault(); if (!linkUserId || !linkOrgId) return
    setLinking(true)
    await supabase.from('profiles').update({ organization_id: linkOrgId }).eq('id', linkUserId)
    setLinking(false); setLinkUserId(''); setLinkOrgId(''); onReload()
  }

  // Unlinked users (no org yet)
  const unlinkedUsers = profiles.filter(p => !p.organization_id)
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
        <button onClick={() => setShowCreate(v => !v)} style={{ display: 'flex', alignItems: 'center', gap: '6px', background: 'var(--success-soft)', border: '1px solid var(--success)', borderRadius: '8px', padding: '9px 14px', color: 'var(--success)', fontSize: '13px', cursor: 'pointer', fontWeight: '600' }}>
          <Plus size={14} /> Nieuwe organisatie
        </button>
      </div>
      ...
      {showCreate && (
        <Card style={{ padding: '20px' }}>
          <div style={{ fontSize: '13px', fontWeight: '600', color: 'var(--text)', marginBottom: '14px' }}>Organisatie aanmaken</div>
          <form onSubmit={createOrg} style={{ display: 'flex', gap: '12px', alignItems: 'flex-end', flexWrap: 'wrap' }}>
            <div style={{ flex: 1, minWidth: '200px' }}>
              <label style={{ display: 'block', color: 'var(--text-2)', fontSize: '11px', fontWeight: '600', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '6px' }}>Naam</label>
              <input style={inp} value={name} onChange={e => setName(e.target.value)} placeholder="Den Hartogh Solutions" required />
            </div>
            <div>
            ...
            <button type="submit" disabled={creating} style={{ background: 'var(--success)', border: 'none', borderRadius: '8px', padding: '9px 16px', color: '#fff', fontSize: '13px', fontWeight: '600', cursor: 'pointer' }}>
              {creating ? 'Aanmaken…' : 'Aanmaken'}
            </button>
            <button type="button" onClick={() => setShowCreate(false)} style={{ background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: '8px', padding: '9px 14px', color: 'var(--text-2)', fontSize: '13px', cursor: 'pointer' }}>
              Annuleren
            </button>
          </form>
        </Card>
      ...
      {/* Gebruiker koppelen aan organisatie */}
      {unlinkedUsers.length > 0 && (
        <Card style={{ padding: '20px', border: '1px solid var(--warning)', background: 'var(--warning-soft)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
            <AlertCircle size={15} style={{ color: 'var(--warning)' }} />
            <div style={{ fontSize: '13px', fontWeight: '600', color: 'var(--text)' }}>
              {unlinkedUsers.length} gebruiker{unlinkedUsers.length !== 1 ? 's' : ''} zonder organisatie
            </div>
          </div>
          <form onSubmit={linkUser} style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'flex-end' }}>
            <div style={{ flex: 1, minWidth: '180px' }}>
              <label style={{ display: 'block', color: 'var(--text-2)', fontSize: '11px', fontWeight: '600', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '5px' }}>Gebruiker</label>
              <select style={inp} value={linkUserId} onChange={e => setLinkUserId(e.target.value)} required>
                <option value="">— Selecteer gebruiker —</option>
                {unlinkedUsers.map(u => <option key={u.id} value={u.id}>{u.full_name || u.email}</option>)}
              </select>
            </div>
            <div style={{ flex: 1, minWidth: '180px' }}>
              <label style={{ display: 'block', color: 'var(--text-2)', fontSize: '11px', fontWeight: '600', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '5px' }}>Organisatie</label>
              <select style={inp} value={linkOrgId} onChange={e => setLinkOrgId(e.target.value)} required>
                <option value="">— Selecteer organisatie —</option>
                {orgs.map(o => <option key={o.id} value={o.id}>{o.name}</option>)}
              </select>
            </div>
            <button type="submit" disabled={linking || !linkUserId || !linkOrgId} style={{ background: 'var(--accent)', border: 'none', borderRadius: '8px', padding: '9px 14px', color: '#fff', fontSize: '13px', fontWeight: '600', cursor: 'pointer', opacity: (!linkUserId || !linkOrgId) ? 0.5 : 1 }}>
      ...
      <Card>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr>{['Organisatie', 'Plan', 'Gebruikers', 'MRR', 'Uitnodigingscode', 'Aangemaakt', 'Acties'].map(h => <th key={h} style={th}>{h}</th>)}</tr>
          </thead>
          <tbody>
            {orgs.length === 0 ? (
              <tr>
                <td colSpan={7} style={{ ...td, textAlign: 'center', color: 'var(--text-3)', padding: '40px' }}>
                  <div style={{ marginBottom: '8px' }}>Nog geen organisaties</div>
                  <div style={{ fontSize: '12px' }}>Maak een nieuwe organisatie aan via de knop hierboven.</div>
                </td>
              </tr>
            ) : orgs.map(o => {
              ...
              return (
                <tr key={o.id}>
                  <td style={td}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                      <div style={{ width: '32px', height: '32px', borderRadius: '8px', background: 'var(--surface-3)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  ...
                  <td style={{ ...td, fontFamily: 'monospace' }}>{memberCount}</td>
                  ...
                  <td style={td}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      ...
                      <button onClick={() => copyCode(o.invite_code)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: copied === o.invite_code ? 'var(--success)' : 'var(--text-3)', display: 'flex', padding: '2px' }}>
                        {copied === o.invite_code ? <Check size={13} /> : <Copy size={13} />}
                      </button>
                      <button onClick={() => regenCode(o.id)} title="Nieuwe code genereren" style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-3)', display: 'flex', padding: '2px' }}>
                        <RefreshCw size={12} />
                      </button>
                    </div>
                  </td>
                  ...
                  <td style={td}>
                    <button onClick={() => deleteOrg(o.id, o.name)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--danger)', display: 'flex' }}>
                      <Trash2 size={14} />
                    </button>
                  </td>
                </tr>
        ...
        <Card style={{ padding: '20px' }}>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={monthlyRevData}>
              <defs><linearGradient id="gMrr" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#10b981" stopOpacity={0.2} /><stop offset="95%" stopColor="#10b981" stopOpacity={0} /></linearGradient></defs>
              <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" vertical={false} />
              <XAxis dataKey="name" tick={{ fontSize: 10, fill: 'var(--text-3)' }} axisLine={false} tickLine={false} />
              <YAxis tick={{ fontSize: 10, fill: 'var(--text-3)' }} axisLine={false} tickLine={false} />
              <Tooltip content={<CustomTooltip />} />
...
export default function AdminPanel() {
  ...
  return (
    <div style={{ display: 'flex', minHeight: 'calc(100vh - 52px)' }}>
      ...
      {/* ── Linker sidebar ──────────────────────────────────────────────────── */}
      <aside style={{ width: '220px', flexShrink: 0, borderRight: '1px solid var(--border)', background: 'var(--surface)', padding: '24px 12px', display: 'flex', flexDirection: 'column', gap: '2px', position: 'sticky', top: 52, height: 'calc(100vh - 52px)', overflowY: 'auto' }}>
        <div style={{ padding: '0 8px', marginBottom: '16px' }}>
          <div style={{ fontSize: '11px', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--text-3)' }}>Platform Beheer</div>
        </div>
        ...
        {NAV.map(({ id, label, icon: Icon, badge }) => {
          ...
          return (
            ...
            >
              ...
              {badge !== undefined && (
                <span style={{ background: active ? 'var(--accent)' : 'var(--surface-3)', color: active ? '#fff' : 'var(--text-3)', borderRadius: '10px', padding: '1px 7px', fontSize: '10px', fontWeight: '700', minWidth: '20px', textAlign: 'center' }}>
                  {badge}
                </span>
              )}
            </button>
        ...
        {/* Vernieuwen knop onderaan */}
        <div style={{ marginTop: 'auto', paddingTop: '16px', borderTop: '1px solid var(--border)' }}>
          <button
            onClick={load}
            disabled={loading}
            style={{ display: 'flex', alignItems: 'center', gap: '7px', width: '100%', padding: '8px 10px', borderRadius: '8px', border: 'none', background: 'transparent', color: 'var(--text-3)', fontSize: '12px', cursor: 'pointer' }}
          >
      ...
      <main style={{ flex: 1, padding: '32px 36px', overflowY: 'auto', minWidth: 0 }}>
        {loading ? (
          ...
          <>
            {activeSection === 'dashboard'  && <DashboardTab  profiles={profiles} orgs={orgs} />}
            {activeSection === 'gebruikers' && <GebruikersTab profiles={profiles} setProfiles={setProfiles} orgs={orgs} onReload={load} />}
            {activeSection === 'orgs'       && <OrgsTab       orgs={orgs} setOrgs={setOrgs} profiles={profiles} onReload={load} />}
            {activeSection === 'financieel' && <FinancieelTab orgs={orgs} profiles={profiles} />}
            {activeSection === 'systeem'    && <SysteemTab    profiles={profiles} orgs={orgs} />}
          </>
        )}
      </main>
    </div>
