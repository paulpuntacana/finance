-- ============================================================
-- DHS Finance — COMPLETE SUPABASE SETUP
-- Plak dit VOLLEDIG in Supabase → SQL Editor → Run
--
-- Dit maakt alles aan vanaf nul (veilig om meerdere keren te draaien).
-- Volgorde: basis → triggers → v3-tabellen → RLS → seed
-- ============================================================
...
-- ── RLS op basis tabellen ─────────────────────────────────────
alter table public.organizations enable row level security;
alter table public.profiles       enable row level security;

drop policy if exists "admin_all_orgs"         on public.organizations;
drop policy if exists "user_own_org"            on public.organizations;
drop policy if exists "authenticated_create_org" on public.organizations;
drop policy if exists "own_profile"             on public.profiles;
drop policy if exists "admin_all_profiles"      on public.profiles;
drop policy if exists "org_members_see_each_other" on public.profiles;
...
create policy "authenticated_create_org" on public.organizations for insert
...
-- ── Auto-profiel trigger bij nieuwe registratie ──────────────
...
-- ============================================================
-- V3 TABELLEN (drop bestaande v2 tabellen eerst)
-- ============================================================
...
drop table if exists public.horizon_scenarios  cascade;
...
-- ============================================================
-- ROW LEVEL SECURITY (v3 tabellen)
-- ============================================================
...
create policy "entities_org"           on public.entities           for all using (org_id = public.user_org_id() or public.is_platform_admin());
create policy "clients_org"            on public.clients            for all using (org_id = public.user_org_id() or public.is_platform_admin());
create policy "invoices_org"           on public.invoices           for all using (org_id = public.user_org_id() or public.is_platform_admin());
create policy "invoice_items_org"      on public.invoice_items      for all using (invoice_id in (select id from public.invoices where org_id = public.user_org_id()) or public.is_platform_admin());
create policy "invoice_reminders_org"  on public.invoice_reminders  for all using (invoice_id in (select id from public.invoices where org_id = public.user_org_id()) or public.is_platform_admin());
create policy "quotes_org"             on public.quotes             for all using (org_id = public.user_org_id() or public.is_platform_admin());
create policy "quote_items_org"        on public.quote_items        for all using (quote_id in (select id from public.quotes where org_id = public.user_org_id()) or public.is_platform_admin());
create policy "purchase_invoices_org"  on public.purchase_invoices  for all using (org_id = public.user_org_id() or public.is_platform_admin());
create policy "expenses_org"           on public.expenses           for all using (org_id = public.user_org_id() or public.is_platform_admin());
create policy "boek_assets_org"        on public.boek_assets        for all using (org_id = public.user_org_id() or public.is_platform_admin());
create policy "boek_entries_org"       on public.boek_entries       for all using (org_id = public.user_org_id() or public.is_platform_admin());
create policy "org_settings_org"       on public.org_settings       for all using (org_id = public.user_org_id() or public.is_platform_admin());
create policy "reminder_templates_org" on public.reminder_templates for all using (org_id = public.user_org_id() or public.is_platform_admin());
create policy "horizon_scenarios_org"  on public.horizon_scenarios  for all using (org_id = public.user_org_id() or public.is_platform_admin());

-- ── Public invoice sign policy (zonder auth) ─────────────────
create policy "invoices_public_sign" on public.invoices for select
  using (sign_token is not null);
create policy "quotes_public_sign" on public.quotes for select
  using (sign_token is not null);
...
-- ============================================================
-- VERIFICATIE — controleer of alles aangemaakt is
-- ============================================================
select table_name from information_schema.tables
where table_schema = 'public'
  and table_name in ('organizations','profiles','entities','clients','invoices',
                     'invoice_items','invoice_reminders','quotes','quote_items',
                     'purchase_invoices','expenses','boek_assets','boek_entries',
                     'org_settings','horizon_scenarios')
order by table_name;
